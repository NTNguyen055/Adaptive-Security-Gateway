...
All gateway behavior is controlled via environment variables in `.env`. No code changes or image rebuilds required.
...
# ── L1 Rate Limit (Nginx Leaky Bucket) ────────────────────────────────
RATE_LIMIT_RPS=30         # Max requests/second per IP
RATE_LIMIT_BURST=60       # Burst capacity above RPS

# ── L2 Rate Limit (Redis Sliding Window) ──────────────────────────────
REDIS_RATE_LIMIT=150      # Max requests per window
REDIS_RL_WINDOW=60        # Window size in seconds

# ── Auto-Blacklist Thresholds ─────────────────────────────────────────
AUTO_BL_THRESHOLD=5       # Rate-limit violations before auto-ban
AUTO_BL_DURATION=3600     # Ban duration in seconds (default: 1 hour)

# ── Brute-Force Protection ────────────────────────────────────────────
BRUTE_FORCE_MAX_ATTEMPTS=5   # Failed attempts before lockout
BRUTE_FORCE_WINDOW=300        # Rolling window in seconds (5 min)
BRUTE_FORCE_RISK_PENALTY=80   # Risk score added on lockout

# ── Risk Engine Thresholds ────────────────────────────────────────────
RISK_LIMIT_THRESHOLD=50   # Risk score → HTTP 429
RISK_BLOCK_THRESHOLD=80   # Risk score → HTTP 403 + permanent ban

# ── IP Reputation Decay (new) ─────────────────────────────────────────
RISK_DECAY_ENABLED=false  # true: forgive shared NAT IPs over time
RISK_DECAY_FACTOR=0.1     # 10% reduction per clean request

# ── Geo Blocking (new) ────────────────────────────────────────────────
GEO_ALLOWED_COUNTRIES=VN,US,SG,JP   # ISO 3166-1 alpha-2 codes
